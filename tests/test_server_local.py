...
def test_process_starts_and_stops_in_context_manager() -> None:
    """Test server startup and shutdown with context manager.

    This test verifies that when using LanguageTool as a context manager, the server
    process is running while inside the context and is properly terminated when exiting
    the context.

    :raises AssertionError: If the server process is not running after creation or if it
        continues running after context manager exit.
    """
    with language_tool_python.LanguageTool("en-US") as tool:
        proc: subprocess.Popen[str] | None = tool._server
        if proc is None:
            err = "tool._server is None after creation"
...
def test_process_starts_and_stops_on_close() -> None:
    """Test server startup and shutdown with explicit close().
    ...
    :raises AssertionError: If the server process is not running after creation or if it
        continues running after close() is called.
    """
    ...
    # Make sure process is running before killing language tool object.
    assert proc.poll() is None, "tool._server not running after creation"
    # Explicitly close() object so process stops before garbage collection.
    tool.close()
    del tool
    # Make sure process stopped after close() was called.
...
def test_local_client_server_connection() -> None:
    ...
    instance can successfully connect to it as a remote client, allowing grammar
    checking to be performed through the client-server architecture.

    :raises AssertionError: If the client cannot successfully check text through the
        server.
    """
...
def test_new_spellins_in_es() -> None:
    ...
    This test verifies that when new_spellings are added for the Spanish language, they
    are correctly recognized by the tool during grammar checking. (This test is
    important to ensure that the new spellings functionality works across different
    languages and is not limited to English.)

    :raises AssertionError: If the new spellings are not recognized in Spanish.
    """
    with language_tool_python.LanguageTool(
        "es",
        new_spellings=["ejempo"],
        new_spellings_persist=False,
    ) as tool:
...
def test_uk_typo() -> None:
    ...
    This test verifies that LanguageTool correctly identifies and corrects grammar
    errors specific to UK English, including proper handling of contractions like
    "you're" and "your", while respecting UK English conventions where "You're" can mean
    "Your" in certain contexts.

    :raises AssertionError: If the detected errors or corrections do not match expected
        UK English rules.
    """
